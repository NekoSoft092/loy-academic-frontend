...
export function MessageComponent(props: IMessage): JSX.Element {
    ...
    const handleGenerateVoice = async (e: any, text: string): Promise<void> => {
        e.preventDefault();

        const response = await fetch('http://localhost:8000/api/v1/messages/generate-voice/', {
            method: 'POST',
            headers: {
                                        ...
                                        { props.direction === 'incoming' && (
                                            <div className={index === 0? 'clipboard-header': 'clipboard-content'}>
                                               ...
                                               <div className="flex gap-2">
                                                ...
                                                {(index === 0 && (
                                                    <i onClick={async(e) => {
                                                        await handleGenerateVoice(e, item.trim())
                                                    }}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
